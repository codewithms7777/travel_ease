...
$admin_exists = mysqli_query($conn, "SELECT id FROM users WHERE role='admin' LIMIT 1");
if ($admin_exists && mysqli_num_rows($admin_exists) === 0) {
    $hash = password_hash($default_admin_password, PASSWORD_DEFAULT);
    $stmt = mysqli_prepare($conn, "INSERT INTO users (full_name, email, password, role, created_at) VALUES (?, ?, ?, 'admin', NOW())");
